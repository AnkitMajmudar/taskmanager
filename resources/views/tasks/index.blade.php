...
document.addEventListener('DOMContentLoaded', function(){
  ...
  function renderTasks(tasks){
    ...
    tasks.forEach(task => {
      ...
      col.innerHTML = `
        <div class="card task-card ${isCompleted} ${priorityClass} h-100">
    ...
    // Attach events
    document.querySelectorAll('.btn-edit').forEach(btn => btn.addEventListener('click', e => openEdit(e.currentTarget.dataset.id)));
    document.querySelectorAll('.btn-delete').forEach(btn => btn.addEventListener('click', e => deleteTask(e.currentTarget.dataset.id)));
    document.querySelectorAll('.btn-toggle').forEach(btn => btn.addEventListener('click', e => toggleStatus(e.currentTarget.dataset.id, e.currentTarget.dataset.completed === 'true')));
  }
  ...
  async function openEdit(id){
    try {
      ...
      if (task.due_date) {
        ...
        document.getElementById('due_date').value = new Date(d.getTime() - d.getTimezoneOffset()*60000).toISOString().slice(0,16);
      } else {
